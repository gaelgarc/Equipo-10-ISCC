...
void guardarPartida(int tablero[4][6], HWND botones[], int puntaje) {
    ...
    for (int i = 0; i < TOTAL_CARTAS; i++) {
        char texto[4];
        GetWindowText(botones[i], texto, sizeof(texto));
        fprintf(file, "%d ", strcmp(texto, "?") == 0 ? 0 : 1);
    }
...
EntradaRanking* obtenerTopRanking(usuario* usuarios) {
    if (cantidad == 0 || usuarios == NULL) return NULL;
...
LRESULT CALLBACK VentanaRanking(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    switch (msg) {
        ...
        case WM_SHOWWINDOW:
            if (wParam == TRUE) {  // La ventana se est� mostrando
...
LRESULT CALLBACK VentanaJuego(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    static BOOL modoDificil = TRUE; // Bandera activada
    ...
    switch (msg) {
        ...
        case WM_COMMAND:
            if (LOWORD(wParam) == 4) { // Iniciar
            ...
            else if (LOWORD(wParam) == 300) { // Regresar
                HWND botonIniciar = GetDlgItem(hwnd, 4);
                ...
                if (contador == 0) {
                    ...
                    if (modoDificil) {
                        SetTimer(hwnd, 2, 800, NULL); // 2 segundos para responder
                    }
                }
                else if (contador == 1 && indice != primerSeleccion) {
                    if (modoDificil) {
                        KillTimer(hwnd, 2); // Cancelar temporizador si se seleccion� a tiempo
                    }

                    segundoSeleccion = indice;
                    contador = 2;
                    SetTimer(hwnd, 1, 1000, NULL); // Comprobaci�n normal
                }
            }
            break;

        case WM_TIMER:
            if (wParam == 1) { // Timer para comparar dos cartas
                KillTimer(hwnd, 1);
            ...
            else if (wParam == 2) { // Timer para modo dif�cil
                KillTimer(hwnd, 2);
